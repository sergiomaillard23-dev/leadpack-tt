-- Migration 007: Add pack_cracks table and payment_type to wallet_transactions
-- Date: 2026-04-09
-- Purpose: Enable 5-minute pack preview windows and wallet top-up functionality

-- Note: wallet_balance already exists on agents table from prior migration

-- 1. Add payment_type column to wallet_transactions table
...
-- 2. Create pack_cracks table to track 5-minute preview windows
...
-- 4. Create function to check if agent has active crack on pack
...
-- 5. Create function to check if agent is locked out (crack expired, not purchased)
...
-- 6. Create function to get remaining seconds on active crack
